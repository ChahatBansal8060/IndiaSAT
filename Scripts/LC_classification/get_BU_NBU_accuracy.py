...
def combine_bu_nbu_tifs(tif_folder_path):
	nbu_list=['Barrenland','Water','Greenery']
	i=0
	for cat in nbu_list:	
...
def ComputeAccuracy(cropped_images_folder, text_file):
	...
	for landcover in landcover_classes:
		landcover_predicted_class_count = [0, 0] # [bu, nbu (Greenery + Water + Barrenland)]
...
def main():
    ...
    accuracy_result_folder = 'Results/BU_NBU_Accuracy_'+year
    os.makedirs(accuracy_result_folder, exist_ok=True)
    text_file_path = accuracy_result_folder+'/'+district_name+'_accuracy_2cat.txt'
